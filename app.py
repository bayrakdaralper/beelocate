...
TRANS = {
    'TR': {
        ...
        'err_summary': "Bölgede baskın bitki örtüsü uydu verileriyle ayırt edilememiştir. Hakim rüzgar <b>{wind}</b> yönündedir. {water_txt}.",
    ...
    'EN': {
        ...
        'err_summary': "Dominant vegetation could not be distinguished via satellite data. Prevailing wind is <b>{wind}</b>. {water_txt}.",
...
def calculate_score(lat, lng, radius=2000, lang="TR"):
    cache_key = f"v2_score_{round(lat,4)}_{round(lng,4)}_{radius}_{lang}"
    cached = cache.get(cache_key)
    if cached: return cached

    try: ox.settings.timeout = 20; gdf = ox.features_from_point((lat, lng), {'natural':['water','wood','scrub','grassland'], 'waterway':True, 'landuse':['forest','orchard','farmland','meadow','grass'], 'building':True, 'highway':['motorway','primary','secondary']}, dist=1500)
    ...
    if not f.empty:
        ...
        if "wood" in ts or "forest" in ts: flora_name = "Orman (Cam/mese)" if lang=="TR" else "Forest"
        elif "meadow" in ts or "grass" in ts: flora_name = "Mera/Cicek" if lang=="TR" else "Meadow"
        elif "farm" in ts: flora_name = "Tarim (Risk)" if lang=="TR" else "Agriculture"
...
def analyze():
    try:
        ...
        # AI Metni Oluştur (PYTHON TARAFINDA)
        L = TRANS[lang]
        ...
        wind_dir_txt = L['wind'].split(' ')[0] + " " + dets['wind_dir'] # Basit yon
        ...
        return jsonify({'error': 'Analiz hatasi.'}), 500
