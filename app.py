...
from flask_caching import Cache  # YENİ: Performans için
import osmnx as ox
...
import numpy as np
import io
import traceback  # YENİ: Hata takibi için
...
# --- CACHE AYARLARI (YENİ) ---
# Sonuçları 10 dakika (600 saniye) boyunca hafızada tutar
...
TRANS = {
    'TR': {
        ...
        'summary': "Bu bolge {score} puan ile {pot} olarak siniflandirilmistir. Bitki ortusu agirlikli olarak {flora} turlerinden olusmaktadir. {water_txt}. {wind_txt}",
    ...
    'EN': {
        ...
        'summary': "This location is classified as {pot} with a score of {score}. Vegetation consists mainly of {flora} types. {water_txt}. {wind_txt}",
        'w_good': "Access to water sources is ideal",
        'w_bad': "Water source is distant, supplementary water may be needed",
...
def create_radar_chart(scores):
    ...
    plt.savefig(tmp_file.name, transparent=True, dpi=100); plt.close()
...
def get_meteo_extended(lat, lng):
    try:
        ...
        return {'cur_temp': c.get('temperature_2m', 0), 'avg_temp': round(np.mean(d.get('temperature_2m_max', [20])), 1),
                'avg_wind': round(np.mean(d.get('wind_speed_10m_max', [5])), 1), 'avg_hum': int(np.mean(d.get('relative_humidity_2m_mean', [50]))),
...
def calculate_score(lat, lng, radius=2000, lang="TR"):
    # CACHE KONTROLÜ: Aynı koordinatlar daha önce soruldu mu?
    # Koordinatları yuvarlıyoruz ki çok yakın tıklamalar aynı sayılsın
    cache_key = f"score_{round(lat, 4)}_{round(lng, 4)}_{radius}_{lang}"
    ...
    if not f.empty:
        ...
        if "wood" in ts or "forest" in ts: flora_name = "Orman (Cam/mese)" if lang=="TR" else "Forest (Pine/Oak)"
        elif "meadow" in ts or "grass" in ts: flora_name = "Mera/Cicek" if lang=="TR" else "Meadow/Flower"
        elif "farm" in ts: flora_name = "Tarim (Risk)" if lang=="TR" else "Agriculture (Risk)"
        else: flora_name = "Calilik" if lang=="TR" else "Scrub"
    
    s_f = 100 if d_flora < 10 else max(0, 100-(d_flora/2000)*100)
    if "Risk" in flora_name: s_f *= 0.6
    
    asp_sc = {'S':100,'SE':100,'SW':90,'E':80,'W':50,'NE':30,'NW':20,'N':10}; s_a = asp_sc.get(aspect_dir, 50)
    ...
    result = (int(total), subs, dets)
    cache.set(cache_key, result) # Sonucu cache'e kaydet
    return result

# --- PDF SINIFI ---
class PremiumReport(FPDF):
    def __init__(self, lang='TR'):
        super().__init__(); self.L = TRANS[lang]
    def header(self):
...
def analyze():
    try:
        ...
        # Grid Isı Haritası
        grid = []; off = rad/111000; offsets = [(0,0),(off/2,0),(-off/2,0),(0,off/2),(0,-off/2)]
        ...
        # GÜVENLİK: Kullanıcıya hatayı gösterme, konsola yaz.
        print("KRİTİK HATA:", traceback.format_exc())
        return jsonify({'error': 'Analiz sirasinda beklenmeyen bir hata olustu. Lutfen tekrar deneyin.'}), 500
...
def download_report():
    try:
        ...
        chart = create_radar_chart(s); pdf.image(chart, x=55, w=100); os.remove(chart); pdf.ln(5)
        ...
        summary = L['summary'].format(score=score, pot=tr_chars(pot), flora=tr_chars(d['flora_type']), water_txt=tr_chars(water_txt), wind_txt=tr_chars(wind_txt))
        ...
        pdf.info_row(L['water'], w_res); pdf.info_row(L['wind'], f"{d['avg_wind']} km/h ({d['wind_dir']})")
